...
description: >
  ...
  AI and automation consultancy. Use this whenever Eric asks to find, search
  for, sweep, hunt, or check tenders, bids, contracts, procurements, RFPs or
  framework opportunities, or mentions Find a Tender, Contracts Finder, CPV
  codes, or a specific public-sector buyer, even when he does not say the
  word "skill" or "sweep". It carries the CPV code set, the FWF keyword
  taxonomy, the triage rules including the financial-standing constraint,
  and the verify discipline that separates live results from cached ones.
  Reach for it on any phrasing like "find me tenders", "what's out there for
  FWF", "anything new on Find a Tender", or "check for Copilot work".
---

# FWF Tender Sweep

Run a structured search of UK public-sector procurement portals, then hand
back a triaged shortlist matched to FWF's Microsoft, AI and automation lane.

## Read this limitation first, and tell Eric plainly
...
If Eric has not specified these, assume the defaults and state the
assumptions in one line rather than stopping to ask.

- Sectors: default all. He may include or exclude (for example NHS and
  central government only, or exclude police and defence).
- Value band: default ignore. He may set a floor or ceiling.
- Results wanted: default the strongest 6 to 8.
- Focus: default the full net. He may narrow it (for example Copilot
  enablement only, or RPA only).
- Entity: this skill is FWF only. If he means Neofortis, stop and confirm,
  because the positioning and constraints differ.
...
sub-threshold work that suits a two-person entity, so include it unless Eric
scopes it out. Ignore paid aggregators completely (Tender Impulse,
...
3. Discard aggregators, closed notices and contract award notices (unless
   Eric asked for awards as pipeline intel), and anything clinical-only or
   hardware-only.
4. Verify (see below).
...
- If a fetch returns 429 or otherwise fails, say so plainly and mark the
  item snippet-level and unverified. Do not retry more than once or two
  times; portal throttling is common and hammering it wastes the turn.
...
UK English, direct, advanced-practitioner level, no softening. Rank by fit
to the Microsoft, AI and automation lane, freshest first. For each item:
