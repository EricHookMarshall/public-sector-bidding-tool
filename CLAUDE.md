...
- **A-series — the standard-answer bank** ✅ `src/answers.py` + `standard_answers`: the questions every bid asks that need **no reasoning** ("do you have a Modern Slavery policy? attach it"). A **lookup, never a generator** — it answers from record with provenance, or it refuses. Question definitions are committed; **values live only in the gitignored `bids.db`**, re-seeded from the library each startup. A readiness gate makes only `ready` auto-fillable, so no answer can route around another entity's certificate, a contradiction, or a lapsed document. See [`docs/standard-answers.md`](docs/standard-answers.md); the source review behind it is [`docs/bid-library-deep-review.md`](docs/bid-library-deep-review.md).
- **Beyond the journey** *(optional / deferred)*: the Answers UI (A2 — the bank is API-only today), `GraphSharePoint` provider (when MS Graph lands), Azure OpenAI provider (`src/llm.py` skeleton), persist AI-draft provenance, pipeline↔CRM (**Pipedrive or HubSpot — the docs disagree; confirm first**). See `_session/handover.md`.
